...
# 2. Cluster-Based Resampling: Systematically partition the data using GMM over a grid of parameters (phi, n_c).
...
gmm = GaussianMixture() # Use GMM for its ability to model non-spherical clusters.
...
def resampling_grid(phi_list, n_c_list, random_state_clustering, single_preprocessing_res_grid, y_train, distance_metrics):
    ...
    for phi in tqdm(phi_list, position=0, leave=True, desc="Phi Loop"):
        ...
        for n_c in tqdm(n_c_list, position=1, leave=True, desc="N_c Loop"):
            gmm_params = {'n_components': n_c, 'random_state': random_state_clustering}
...
# Persist the resampled data subsets.
